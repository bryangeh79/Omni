// Meta WhatsApp Business Platform channel config routes (Phase 7A).
...
function safeChannelView(ch: {
  ...
  lastWebhookAt?: Date | null; createdAt: Date; updatedAt: Date;
}) {
  return {
    id:                    ch.id,
...
export async function metaChannelRoutes(app: FastifyInstance) {
  ...
  app.post<{
    ...
    const { tenantId } = getAuthUser(req)
    const { displayName, phoneNumberId, wabaId, displayPhoneNumber, metaAccessToken, webhookVerifyToken } = req.body ?? {}

    if (!phoneNumberId?.trim()) {
      return reply.status(400).send({ error: 'phoneNumberId is required' })
    }
    if ((metaAccessToken || webhookVerifyToken) && !isVaultConfigured()) {
  ...
  app.post<{
    Params: { id: string }
    Body:   { metaAccessToken?: string; webhookVerifyToken?: string }
    ...
    const { metaAccessToken, webhookVerifyToken } = req.body ?? {}
    if (!metaAccessToken && !webhookVerifyToken) {
      return reply.status(400).send({ error: 'metaAccessToken or webhookVerifyToken is required' })
    }
  ...
  // ── DELETE /channels/meta/:id/token — clear stored tokens ─────────────────
  app.delete<{ Params: { id: string } }>(
    '/:id/token', { preHandler: requireAuth }, async (req, reply) => {
      ...
      return { ...safeChannelView(updated), message: 'All tokens cleared.' }
    },
  )

  // ── POST /channels/meta/:id/test-config-dry-run ────────────────────────────
  // Validates local config shape only. Does NOT call Meta API. Safe for smoke test.
