...
// ── 计费套餐 (Plan ID / name) ────────────────────────────────────────────────
// Round-5：套餐 ID 是稳定 API 值，本映射仅作 UI 显示用，不修改任何 API payload。
export const PLAN_LABELS: Record<string, string> = {
  ...
  growth:     'Pro 成长版',
  GROWTH:     'Pro 成长版',
  business:   'Business 企业版',
  BUSINESS:   'Business 企业版',
  enterprise: 'Business 企业版',
  ENTERPRISE: 'Business 企业版',
  trial:      '试用版',
  TRIAL:      '试用版',
  free:       '免费版',
...
export function planLabel(value: unknown): string {
  if (value === null || value === undefined || value === '') return '未设置套餐'
  const key = String(value)
  return PLAN_LABELS[key] ?? PLAN_LABELS[key.toLowerCase()] ?? PLAN_LABELS[key.toUpperCase()] ?? key
}
...
export function planPeriodLabel(value: unknown): string {
  if (value === null || value === undefined || value === '') return '—'
  const key = String(value)
  return PLAN_PERIOD_LABELS[key] ?? PLAN_PERIOD_LABELS[key.toLowerCase()] ?? key
}

// ── 上线 / 发布 / QA 状态 ────────────────────────────────────────────────────
// Round-5：合并 launch / release / qa 三类状态枚举，单一映射表
export const LAUNCH_STATUS_LABELS: Record<string, string> = {
  // launch
  NOT_READY:                    '尚未就绪',
  READY_FOR_STAGING:            '可进入预演环境',
  READY_FOR_PRODUCTION_REVIEW:  '可进入生产复核',
  READY_FOR_OPERATOR_REVIEW:    '可进入人工复核',
  // checklist item
  DONE:       '已完成',
  COMPLETE:   '已完成',
  COMPLETED:  '已完成',
...
export function launchStatusLabel(value: unknown): string {
  return pick(LAUNCH_STATUS_LABELS, value)
}

// QA 与 release 复用同一张映射（值集合是 LAUNCH_STATUS_LABELS 的子集）
export function qaStatusLabel(value: unknown): string {
  return pick(LAUNCH_STATUS_LABELS, value)
}
export function releaseStatusLabel(value: unknown): string {
  return pick(LAUNCH_STATUS_LABELS, value)
}
