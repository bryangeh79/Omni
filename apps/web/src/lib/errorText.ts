...
export function toChineseError(err: unknown, fallback = '操作失败，请稍后再试'): string {
  ...
  // 计费 / 套餐相关 (Round-5)
  if (lower.includes('plan_not_eligible') || lower.includes('plan not eligible') || lower.includes('not eligible')) {
    return '当前套餐不支持此操作，请联系管理员升级套餐'
  }
  if (lower.includes('payment_not_configured') || lower.includes('payment not configured')) {
    return '支付尚未配置；当前仅支持草稿套餐选择'
  }
  if (lower.includes('billing_disabled') || lower.includes('billing disabled')) {
    return '计费功能当前处于安全草稿模式'
  }
  if (lower.includes('payment required') || (/(^|\W)402(\W|$)/.test(raw))) {
    return '需要升级套餐或联系管理员'
  }
  if (lower.includes('quota exceeded') || lower.includes('usage limit') || lower.includes('over quota')) {
    return '当前用量已达上限，请升级套餐或联系管理员'
  }
  if (lower.includes('subscription inactive') || lower.includes('subscription expired')) {
    return '当前订阅未激活，请联系管理员处理'
  }
