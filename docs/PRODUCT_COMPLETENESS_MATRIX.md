...
| i18n (zh / en / ms) full multilingual | UAT Round-1 ~ 5 已完成 **中文优先（Chinese-first）** 全量覆盖（onboarding / channels / knowledge / launch / release / qa / ops / activation / account / audit / billing / team / settings / inbox / pwa / signup / boss）；Round-3 新增 `lib/errorText.ts`、Round-4 新增 `lib/enumLabels.ts`；Round-5 扩展 plan / launch / release / qa 状态映射 + 套餐套餐 ID 中文显示 + 计费错误中文化；Round-6 消除菜单切换时登录页闪烁（三态 authed）；Round-7 重组侧边栏 IA — 租户日常 4 组在顶部、SaaS Admin · 平台运维 1 组在底部分隔；Round-8 产品智能配置中心 + AI 成交配置生成器（多产品 / 文件上传 / 一键生成 FAQ 草稿 / 销售话术 / 评分 / 跟进 / 转人工规则）；i18n 多语言切换框架延后到 post-v1 | Medium |
| Product material file parsing (PDF / DOCX / OCR for images) | Round-8 文件上传 UI 已就绪：accept PDF/DOC/DOCX/TXT/MD/images，记录文件名 + 大小 + MIME；.txt/.md 文本提取到粘贴区。PDF/DOCX/图片提示同时粘贴关键文字以生成 FAQ。生产级 PDF/DOCX 解析（pdf-parse / mammoth）与 OCR 延后避免引入重依赖与安全风险 | Medium |
| Real AI provider for product sales-config generation | Round-8 生成器为确定性 stub（无外部调用，默认安全）；未来若启用真实 AI（需 OMNI_ENABLE_ONBOARDING_AI=true 与 provider key），可在 generator 内增加 mode='ai' 分支，与现有 `tryAiPreviewGeneration` 同样的 fallback 模式 | Medium |
