...
>
> **Post-v1 Round-9I (2026-05)：** FAQ 结构分组 + 弹性 FAQ 数量 + 客户入口菜单预览 + 租户友好规则展示。UAT 反馈：Round-8 generator 强制生成 30-50 条 FAQ，资料不全时大量 "资料中未明确说明，建议转人工确认。"，UX 糟糕。本轮：(1) **弹性 FAQ 数量**：generator 计算 completeness 分（0/8 ~ 8/8），minimal (0-2 → ~10 条) / moderate (3-5 → ~20 条) / complete (6-8 → ~35 条)；优先填充有把握回答的 FAQ，仅当 target 较大时少量回填（25% cap）"建议人工确认"型 FAQ，**minimal 层完全不回填**。(2) **FAQ 分组**：新 `CompanyFaqDraft` 类型 + `generateCompanyFaqs(profile)` 公司级 10 条 FAQ（你们是什么公司 / 几点营业 / 在哪里 / 找真人 / 是机器人吗 / 帮我什么 / 支持语言 / 投诉退款 / 闲聊 / 不知问什么）；产品 FAQ 仍 per-product（`buildFaqDrafts` 已 per-product）；不混入一个列表。(3) **新端点**：`POST /onboarding/general-faq/generate`（公司 FAQ）、`POST /onboarding/customer-entry-menu/preview`（客户入口菜单：语言选择 zh/en/ms + 产品选择 + 找真人客服，**永不**发 WhatsApp，`previewOnly:true` + `realWhatsAppSent:false` + `realMetaCalled:false`）。(4) **save-faq-to-knowledge 扩展**接受 `faqType: 'GENERAL' | 'PRODUCT'`（默认 PRODUCT 向后兼容），GENERAL → `KnowledgeItemType.GLOBAL_FAQ` 不加产品前缀；PRODUCT 维持现有行为。(5) **租户友好规则**：`ProductSalesConfig.tenantFriendlyRules` 5 个 section（AI 会帮你收集的客户资料 / 系统会自动打的客户标签 / AI 意向判断 / 自动跟进草稿，启用前不会真实发送 / 以下情况 AI 会建议转人工），头条用中文人话；技术 trigger key 仅在 `<details>` "查看详细规则（高级 · 可选）"内可见。(6) **missingDataGuidance** 卡：headline + Chinese-label items (适合客户/核心卖点/价格 / 套餐/购买流程/客户需要提供的资料) + ctaLabel "去补充资料"；`PRODUCT_FIELD_LABELS_ZH` 单一事实来源。(7) **summary 扩展**：`missingFieldLabels` / `completenessTier` / `completenessScore` / `completenessMax` / `missingInfoFaqCount`；前端"资料完整度"卡按 tier 着色（complete=emerald / moderate=blue / minimal=amber）。(8) **FAQ 列表 UI**：hasMissingInfo 项 amber 底色 + "建议补充资料" 标签 + 默认 unchecked；header 显示 "产品 FAQ · {productName}（已生成 N 条草稿，其中 M 条建议人工确认）"。Smoke 新增 8 个 block / ~40 check (test 308-315)：minimal data FAQ ≤14 条 + missingInfo ≤30% + 不强制 30 条 ✓ / missingDataGuidance 含中文 label 不暴露 raw key (suitableCustomers 等) ✓ / complete data ≥15 条且 > minimal ✓ / tenantFriendlyRules 中文 headlines 不暴露 raw trigger (ai_low_confidence / PRICE_ASKED_NO_REPLY 等) ✓ / company FAQ endpoint 含 5 条预期问题 ✓ / customer entry menu previewOnly+realWhatsAppSent:false+zh/en/ms+找真人客服+产品列表 ✓ / save GENERAL faqType=GENERAL → 201 + saved≥1 ✓ / minimal data missing-info FAQ ≤ 3 ✓。**未触碰**：真实 AI / Meta / WhatsApp / 邮件 / 支付，全部 false；客户入口菜单仅配置 + 预览，从不连接 WhatsApp 发送层。
